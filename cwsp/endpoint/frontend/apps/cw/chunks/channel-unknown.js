import { At as clearAllCache, Cn as registerComponent, Ct as clearIconCaches, Dn as unifiedMessaging, Dt as ensureStyleSheet, Hn as loadAsAdopted, Ln as BROADCAST_CHANNELS, Ot as reinitializeRegistry, Sn as processInitialContent, Tt as testIconRacing, Ut as getCachedComponent, bn as hasPendingMessages, c as createFileHandler, cn as H, gt as loadSettings, vn as createMessageWithOverrides, wt as debugIconSystem, xn as initializeComponent, yn as enqueuePendingMessage } from "../com/app.js";
import { P as fetchSwCachedEntries, a as createTemplateManager } from "../com/service.js";
	...
	*/
	...
	const renderMarkdownViewer = async () => {
      ...
      <span>Loading Markdown Viewer...</span>
		...
		try {
			const viewer = (await getCachedComponent("markdown-viewer", () => import("./viewer.js"), { componentName: "MarkdownViewer" })).component.createMarkdownView({
	...
	const renderRichEditor = async () => {
      ...
      <span>Loading Rich Editor...</span>
		...
		content.append(loadingElement);
		try {
			const editor = (await getCachedComponent("quill-editor", () => import("./QuillEditor.js"), { componentName: "QuillEditor" })).component.createQuillEditor({
				initialContent: state.markdown || "",
				onContentChange: (content) => {
					state.markdown = content;
					persistMarkdown();
	...
	const renderHistoryView = async () => {
      ...
      <span>Loading History...</span>
		...
		content.append(loadingElement);
		try {
			const historyManager = (await getCachedComponent("history-manager", () => import("../com/app.js").then((n) => n.s), { componentName: "HistoryManager" })).component.createHistoryManager();
	...
	if (typeof BroadcastChannel !== "undefined") try {
		...
		new BroadcastChannel("file-explorer").addEventListener("message", (event) => {
			const message = event.data;
		...
		new BroadcastChannel("print-viewer").addEventListener("message", (event) => {
			const message = event.data;
	...
	const runPrompt = async (promptText, customAIFunction) => {
		...
		try {
			const res = customAIFunction ? await customAIFunction(input, { useActiveInstruction: true }) : await (await import("./unified.js")).recognizeByInstructions(input, instructions);
		...
		try {
			...
			const renderer = {
				"settings": async () => {
					...
					const settingsEl = (await loadComponent("settings", () => import("./settings.js"), { componentName: "Settings" })).component.createSettingsView({
				...
				"file-explorer": async () => {
					...
					await loadComponent("file-explorer", () => import("./explorer.js"), { componentName: "FileManager" });
	...
	const attachToolbarListeners = () => {
		toolbar.addEventListener("click", async (e) => {
			...
			if (action === "export-docx") {
				const md = state.markdown || "";
				if (md.trim()) {
					const { downloadMarkdownAsDocx } = await import("../com/app.js").then((n) => n.m);
