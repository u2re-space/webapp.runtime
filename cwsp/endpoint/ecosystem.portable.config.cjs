...
const resolvePortableConfigPath = () => {
    ...
    const source = isPortableArg(cli) ? cli : isPortableArg(env) ? env : "./portable.config.json";
...
const envFromFile = Object.assign({}, launcherEnv, {
    CWS_PORTABLE_CONFIG_PATH: portableConfigPath || "./portable.config.json",
    CWS_PORTABLE_DATA_PATH: portableDataPath || "./.data"
});
