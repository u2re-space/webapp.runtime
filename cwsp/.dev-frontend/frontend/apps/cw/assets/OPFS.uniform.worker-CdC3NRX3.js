(function() {
	...
	*/
	const workerCode = new URL("" + new URL("Worker-BCSbBPGz.ts", self.location.href).href, "" + self.location.href);
	...
	*/
	...
	function loadWorker(WX) {
		...
		if (typeof WX === "string") {
			if (WX.startsWith("/")) return new Worker(new URL(WX.replace(/^\//, "./"), self.location.href).href, { type: "module" });
			if (URL.canParse(WX) || WX.startsWith("./")) return new Worker(new URL(WX, self.location.href).href, { type: "module" });
